...
**Not verified, and not claimed.** The whole device matrix — including the 2h device run, performance and memory sampling, QR pairing, real-provider combinations and backup/export behaviour — is `PENDING DEVICE` and is the user's to run. No performance, latency or memory figure is claimed anywhere. Three automatic items stay open and are recorded in the phase reports: the phase 1 phone test could not observe the live bridge's request over real HTTP in the Robolectric lane; the attempt budget is not absolutely unbreakable across a seek because the plan's own out-of-window prune drops spent records; and per-word timing is not implemented. Partial-batch repair remains `NOT IMPLEMENTED` with its restart conditions. `DeterministicSegmentationFallback` has no test reference at all.

**Exact-SHA CI: run, and green.** Pushed on 2026-09-14 (the run's `PENDING PUSH AUTHORIZATION` is discharged for these commits). Branch `feature/ai-bilingual-subtitles` at `dd9c9df10`, remote `CometDash77/smarttube` (note: the repository was renamed from `smartube`, which is what this workspace's remote URL still says; git follows the rename redirect, the API does not).

| Ref | Run | Result |
|---|---|---|
| `refs/heads/feature/ai-bilingual-subtitles` `dd9c9df10` | [34864623571](https://github.com/CometDash77/smarttube/actions/runs/34864623571) | success, 6m16s — JUnit JDK 17 (test, lint, assemble), JDK 11 preference tests, signed APK set uploaded |
| tag `ai-subtitle-test-2026.09.14` | [34865334651](https://github.com/CometDash77/smarttube/actions/runs/34865334651) | success, 6m29s — same two jobs plus the pre-release publish, which verifies all four signed APKs and the uploaded SHA-256 sums before un-drafting |

Pre-release published: <https://github.com/CometDash77/smarttube/releases/tag/ai-subtitle-test-2026.09.14> — `SmartTube_beta_32.45_{universal,arm64-v8a,armeabi-v7a,x86}.apk` plus `SHA256SUMS.txt`, signed with the persistent test key. This is the artifact for manual device testing; installing it and running the device matrix is the user's step, and nothing in this ledger reports a device result.

**Nothing in this note is a release statement.** The code package is reproducible from the commits above. A *test pre-release* was explicitly authorized and published for device testing (above); that is not a release decision, and no stable release, tag or artifact beyond that pre-release exists.
...
- M09 remaining: exact-SHA CI for M09's own candidate `2f98945e3` (`PENDING PUSH AUTHORIZATION`) and the entire device matrix including the 2h device run and performance sampling (`PENDING DEVICE`). CI **has** since run green for the later candidate `dd9c9df10` — see the correction run's completion note above — but that is a different tree and does not stand in for the M09 candidate. The two PARTIAL automatic items are both closed by the M07–M09 correction run — the explicit 50-drag case in phase 1, the repeated-terminal-callback assertion in phase 3 — and the M09 report's rows are annotated accordingly.
