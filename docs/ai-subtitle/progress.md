...
- Current milestone: M01 — Repository Reconnaissance & Architecture Baseline
- Current task: `PHASE0-APPROVAL`
- Gate: waiting for one overall approval of Architecture + Migration Map + Roadmap
...
|---|---|---|---|
| Phase 0 reconnaissance | Complete, approval pending | Current Phase 0 documentation commit; resolve from Git history | Read-only evidence and document consistency checks |
...
3. The current GitHub Actions workflow does not automatically validate the feature branch or explicitly invoke unit tests. `M02-T01` will add an isolated validation workflow after approval.
...
- Proposed: feature-owned package tree inside `common`, not a new Gradle module.
- Proposed: clean-room behavioral implementation from KissTranslator evidence.
- Proposed: five Provider Types over two shared protocol adapters.
- Proposed: one decorated cue through existing `SubtitleView`; no renderer fork.
- Proposed: full-track source adapter using existing MediaServiceCore access, with one conditional loader hook only if evidence requires it.
- Proposed: dedicated versioned `AiSubtitleData`, separate from `PlayerData`.
...
After overall approval, create and persist `docs/ai-subtitle/tasks/M02-T01.md`, containing the first self-contained Worker Task Package: add a feature-owned GitHub Actions validation lane. The Commander will not call the Worker; the user transports the package.
...
- First authoritative CI baseline: pending `M02-T01` and a GitHub Actions run on its commit.
