...
| C3 | `onContextEnabledChanged` rebuilds the live session instead of short-circuiting on an unchanged session identity. `AiSubtitleRuntime.applyToBridge` is now the single entry that applies the resolved provider/prompt plus every stored playback setting, and the phone `saveDraft` calls it once. Before the change the phone path called two separate entries (`applySchedulingToBridge` then `applyToBridge`), so the resolved provider/prompt and the scheduling limits *did* apply — but the bilingual order, the context switch and the streaming switch were written to storage and never reached the running bridge. |
