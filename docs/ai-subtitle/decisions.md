...
Status: Accepted; M04-C7 backup/export correction implemented

Date: 2026-09-12

Decision: Provider credentials never live inside serializable profile data, and a dedicated `SecretStore` owns them. The API 23+ path encrypts each secret with an AES-256-GCM key generated in AndroidKeyStore; API 17–22 retains the documented app-private plaintext compatibility exception. Actual records are feature-owned files: API 21+ uses `getNoBackupFilesDir()` and API 17–20 uses `getCacheDir()`. Cache cleanup on the latter band intentionally removes the secret and requires re-entry. Any read or decryption failure normalizes to a configuration/auth failure and Source-Only Fallback. Legacy named `SharedPreferences` records migrate one reference at a time after a successful new-file write, then are removed; delete clears both locations.

Reason: The app floor is API 17 (`SharedModules/constants.gradle`) while [`KeyGenParameterSpec` was added in API 23](https://developer.android.com/reference/android/security/keystore/KeyGenParameterSpec), and [AOSP records symmetric AES/HMAC support as an Android 6.0 addition](https://source.android.com/docs/security/features/keystore#android_60). Android documents Keystore keys as [non-exportable](https://developer.android.com/privacy-and-security/keystore#security-features), but Auto Backup [includes `SharedPreferences` by default](https://developer.android.com/identity/data/autobackup#Files) and excludes the no-backup directory. The official AndroidX encrypted-preferences reference specifically [warns against backing up encrypted preferences because the restore key is likely absent](https://developer.android.com/reference/androidx/security/crypto/EncryptedSharedPreferences) and directs apps to exclude them. File storage eliminates the default-preference exposure without a host Manifest change.
...
Consequences: Legacy-band devices retain the explicitly documented weaker protection. On API 17–20, cache cleanup also clears the credential by design; it is a recoverable re-entry path. The no-backup directory protects API 21+ feature files from Auto Backup without widening the host Manifest. The legacy cache choice is not a universal guarantee against every device-specific export mechanism; the residual risk and review trigger are recorded in `docs/ai-subtitle/research/g04-1-android-secret-storage.md`.
...
- ADR-012's API 17–20 cache fallback carries a documented residual export/availability risk; revisit it if the minimum SDK or backup policy changes.
